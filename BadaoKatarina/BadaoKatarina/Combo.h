...
inline void ComboOnGameUpdate()
{
	...
	if (Player->HasItemId(3146))
	{
		IInventoryItem* hextech = GPluginSDK->CreateItemForId(3146, 700);
		IUnit* target = SelectTarget(SpellDamage, 700);
		if (IsValidTarget(target))
		{
			hextech->CastOnTarget(target);
		}
	}
	if (Player->HasItemId(3144))
	{
		IInventoryItem* bilgewater = GPluginSDK->CreateItemForId(3144, 550);
